  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result && result.topicSummary && !result.topicSummary.startsWith("Sunucu yoğun olabilir") && !result.topicSummary.startsWith("AI modeli")) {
        ...
        const errorMessage = result?.topicSummary || (userProfile?.isAdmin ? "Yapay zeka bir özet üretemedi." : GENERIC_USER_ERROR_MESSAGE);
      ...
      const errorMessageForUser = userProfile?.isAdmin ? (error.message || "Konu özetlenirken beklenmedik bir hata oluştu.") : GENERIC_USER_ERROR_MESSAGE;
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <ScrollArea className="h-auto max-h-[500px] w-full rounded-md border p-4 bg-muted/30">
                ...
                {summaryOutput.yksConnections && summaryOutput.yksConnections.length > 0 && (
                    <>
                    <h3 className="font-semibold text-foreground mt-4">Bağlantılar/Stratejiler:</h3>
                    <ul className="list-disc pl-5">
             ...
             {summaryOutput.sourceReliability && (
                <p className="text-xs text-muted-foreground mt-3 italic">Bilgi Notu: {summaryOutput.sourceReliability}</p>
            )}
      ...
      {!isSummarizing && !summaryOutput && !userProfileLoading && (userProfile || !userProfile) &&(
        <Alert className="mt-6">
          ...
          <AlertDescription>
            Yukarıdaki metin alanına bir konu veya metin girerek hızlı ve YKS odaklı özetinizi alın.
          </AlertDescription>
        </Alert>
      )}
    </div>

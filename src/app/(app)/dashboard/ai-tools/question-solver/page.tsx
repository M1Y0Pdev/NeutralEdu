...
interface ParsedSolution {
  answerOnly: React.ReactNode[] | null;
  problemContext: ParsedSolutionPart[];
...
const formatSolverOutputForDisplay = (text: string | undefined | null): ParsedSolution => {
  if (!text) return { answerOnly: null, problemContext: [], explanationSections: [] };
  const lines = text.split('\n');
  let answerOnly: React.ReactNode[] | null = null;
  const problemContext: ParsedSolutionPart[] = [];
  const explanationSections: ParsedSolutionPart[] = [];
  
  let section: 'answer' | 'context' | 'explanation' = 'answer';
  let inCodeBlock = false;
  let codeBlockContent = "";

  const flushCodeBlock = () => {
    if (inCodeBlock) {
      const targetSection = section === 'explanation' ? explanationSections : problemContext;
      targetSection.push({ type: "codeblock", content: [codeBlockContent.trimEnd()] });
      codeBlockContent = "";
      inCodeBlock = false;
    }
  };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (section === 'answer' && trimmedLine.toLowerCase().startsWith("cevap:")) {
      const answerText = trimmedLine.substring("cevap:".length).trim();
      answerOnly = parseInlineFormatting(answerText);
      section = 'context'; // Move to context after finding the answer line
      continue;
    }

    if (trimmedLine.toLowerCase().startsWith("açıklama:") || trimmedLine.toLowerCase().startsWith("çözüm yolu (adım adım):")) {
      flushCodeBlock();
      section = 'explanation';
    ...
    const currentTargetArray = section === 'explanation' ? explanationSections : problemContext;
    
    if (trimmedLine.startsWith('### ')) {
      flushCodeBlock();
      currentTargetArray.push({ type: "subheading", content: parseInlineFormatting(trimmedLine.substring(4)) });
    } else if (trimmedLine.startsWith('## ')) {
      flushCodeBlock();
      currentTargetArray.push({ type: "heading", content: parseInlineFormatting(trimmedLine.substring(3)) });
    } else if (trimmedLine.startsWith('* ') || trimmedLine.startsWith('- ')) {
      flushCodeBlock(); // Flush before starting a new list, or handle lists better
      currentTargetArray.push({ type: "listitem", content: parseInlineFormatting(trimmedLine.substring(trimmedLine.indexOf(' ') + 1)) });
    } else if (trimmedLine !== "" && (section === 'context' || section === 'explanation')) {
      flushCodeBlock();
      currentTargetArray.push({ type: "paragraph", content: parseInlineFormatting(line) });
    }
  }
  flushCodeBlock();

  return { answerOnly, problemContext, explanationSections };
};
          ...
          <CardContent>
            <ScrollArea className="h-auto max-h-[600px] w-full rounded-md border p-4 bg-muted/30">
              <div className="prose prose-sm dark:prose-invert max-w-none text-muted-foreground">
                {parsedSolution.answerOnly && (
                  <div className="mb-4 p-3 border-b border-border">
                    <h3 className="text-lg font-semibold text-primary mb-1">Cevap:</h3>
                    <div className="text-lg font-bold text-foreground">{parsedSolution.answerOnly}</div>
                  </div>
                )}
                {parsedSolution.problemContext && parsedSolution.problemContext.length > 0 && (
                  <div className="mb-4 p-3 border-b border-border">
                    <h3 className="text-lg font-semibold text-primary mb-1">Soru Hakkında:</h3>
                    {parsedSolution.problemContext.map((part, index) => {
                        let key = `pctx-${index}-${Math.random()}`;
                        switch(part.type) {
                            case "heading": return <h3 key={key} className="text-md font-semibold mt-3 mb-1 text-foreground">{part.content}</h3>;
                            case "subheading": return <h4 key={key} className="text-sm font-semibold mt-2 mb-1 text-foreground">{part.content}</h4>;
                            case "listitem": return <li key={key} className="ml-4 list-disc">{part.content}</li>;
                            case "codeblock": return <pre key={key} className="bg-background/50 p-2 my-2 rounded-md overflow-x-auto text-xs"><code className="text-foreground">{part.content}</code></pre>;
                            case "paragraph": default: return <p key={key} className="mb-1 last:mb-0">{part.content}</p>;
                        }
                    })}
                  </div>
                )}
                {parsedSolution.explanationSections && parsedSolution.explanationSections.length > 0 && (
                   <div className="mb-4 p-3">
                     <h3 className="text-lg font-semibold text-primary mb-1">Açıklama / Çözüm Yolu:</h3>
                    {parsedSolution.explanationSections.map((part, index) => {
                        let key = `expl-${index}-${Math.random()}`;
                        switch(part.type) {
                            case "heading": return <h3 key={key} className="text-md font-semibold mt-3 mb-1 text-foreground">{part.content}</h3>;
                            case "subheading": return <h4 key={key} className="text-sm font-semibold mt-2 mb-1 text-foreground">{part.content}</h4>;
                            case "listitem": return <li key={key} className="ml-4 list-disc">{part.content}</li>;
                ...
                {!parsedSolution.answerOnly && (!parsedSolution.problemContext || parsedSolution.problemContext.length === 0) && (!parsedSolution.explanationSections || parsedSolution.explanationSections.length === 0) && answer?.solution && (

...
export default function TestGeneratorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    resetTestState(); // Call this first to nullify testOutput and show loading
    ...
    try {
      ...
      if (result && result.questions && result.questions.length > 0) {
        ...
        const errorMessage = result?.testTitle || (userProfile?.isAdmin ? "Yapay zeka bir test üretemedi veya format hatalı." : GENERIC_USER_ERROR_MESSAGE);
      ...
      let displayErrorMessage = userProfile?.isAdmin ? (error.message || "Test oluşturulurken beklenmedik bir hata oluştu.") : GENERIC_USER_ERROR_MESSAGE;
  ...
  // Loading screen when isGenerating is true (testOutput is nulled by resetTestState)
  if (isGenerating) {
    return (
  ...
  // Test active and has questions
  if (testOutput && testOutput.questions && testOutput.questions.length > 0 && currentQuestion && !isTestFinished) {
    return (
         <div className="space-y-6">
            <Card className="shadow-sm">
              {/* Header Card for title/description - no changes needed here based on request */}
                <CardHeader>
  ...
  // Initial state or failed test generation state (form + relevant alert)
  return (
    <div className="space-y-6">
      <Card className="shadow-sm">
        <CardHeader>
      ...
      {/* Alert for "Test Oluşturulamadı" - shows only if a test was attempted and failed */}
      {testOutput && testOutput.questions && testOutput.questions.length === 0 && !isGenerating && !isTestFinished && (
        <Alert variant="destructive" className="mt-6">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>Test Oluşturulamadı</AlertTitle>
          <AlertDescription>
            Yapay zeka, girdiğiniz konu ve ayarlarla ilgili soru üretemedi. Lütfen konuyu daha spesifik hale getirmeyi veya farklı ayarlar denemeyi düşünün.
            {testOutput.testTitle && !testOutput.testTitle.toLowerCase().includes("hata") && testOutput.testTitle !== GENERIC_USER_ERROR_MESSAGE && (
              <p className="mt-1 text-xs">AI Mesajı: {testOutput.testTitle}</p>
            )}
            <Button onClick={resetTestState} variant="outline" size="sm" className="mt-3">
              <RotateCcw className="mr-2 h-4 w-4" /> Tekrar Dene / Yeni Ayarlar
            </Button>
          </AlertDescription>
        </Alert>
      )}

      {/* Placeholder Alert for initial state */}
      {!testOutput && !isGenerating && !isTestFinished && !userProfileLoading && (userProfile || !userProfile) &&(
      ...
      {/* Disclaimer shown if a test was attempted (success or fail) or finished */}
      {(testOutput || isTestFinished) && (
